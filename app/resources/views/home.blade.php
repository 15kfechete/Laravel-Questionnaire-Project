...
<? // For Loop for Questionnaires ?>
<div class="card">
    <div class="card-divider">
        {{ $questionnaire->questionnaireTitle }}
        <? // Calls Questionnaire Title ?>
    </div>
    <div class="card-section">
        ...
        <div class="grid-x">
            ...
            <div class="cell small-2">
                <form action="/questionnaires/{{ $questionnaire->id }}" method="post">
                    <? // Form that calls for the DELETE function?>
                    @method('DELETE')
                    <? // Replaces 'post' with DELETE method?>
                    @csrf
                    <? // Cross Site Request ?>
                    <button type="submit" class="alert button">Delete Questionnaire</button>
                    <? // Delete button removes Questionnaire?>
                </form>
            </div>
        </div>
        <div class="grid-x">
            <div class="cell small-2">
                ...
                <? // Link to Access the Specific Questionnaire?>
            </div>
            <div class="cell small-8">
